...
export function audienceProfile(lead) {
  ...
  if (/(szkol|szkoł|uczni|klas|edukacyj|kolonie|obozy|młodzież|mlodziez|dzieci)/i.test(text)) {
    return {
      ...
      fit: "Dla szkoły zrobiłabym wersję bez alkoholu: obiad, słodkie Štramberské ucho z kremem, lekki quiz i małą paczkę oryginalnych uszu na drogę.",
  ...
  if (/(senior|65\+|emeryt|dojrzał|dojrzal|60plus|seniorka)/i.test(text)) {
    return {
      ...
      fit: "Dla seniorów przygotowałabym obiad bez pośpiechu, Štramberské ucho z naszym kremem, lekki quiz przy stole i paczkę uszu na drogę.",
  ...
  if (/(autokar|bus|przewoz|transport|wyjazdy autokarowe|wynajem autobus)/i.test(text)) {
    return {
      ...
      fit: "Dla autokaru najważniejsze jest, żeby wszystko było ustalone wcześniej: godzina, menu, czas obsługi i krótki program, który nie rozbije trasy.",
  ...
  return {
    ...
    fit: "Propozycja jest prosta: grupa przyjeżdża na umówioną godzinę, dostaje obiad, ma krótki lokalny program i może jechać dalej.",
...
export function trelloCardDescription(lead, offer, preparedDraft = null) {
  ...
  const aiSource = draft.source === "openai"
    ...
    : draft.source === "fallback"
      ? "Nahradni pravidlova verze bez OpenAI"
      : "Pravidlova verze";
  return [
    "## Kontakt",
    "",
