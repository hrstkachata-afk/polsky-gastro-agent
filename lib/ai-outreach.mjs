...
const stylePath = path.join(root, "data", "ai-styl.json");
...
function validateDraft(value, fallback, source) {
  ...
  if (subject.length < 8 || body.length < 350) {
...
function buildPrompt({ lead, offer, fallback, pageContext, styleGuide }) {
  return [
    ...
    "- piš kratší e-mail, ne dlouhý text; drž se stylového návodu níže",
    "",
    "Stylový návod od majitele:",
    ...
    "  \"body\": \"polský e-mail; 120-190 slov; konkrétní program a menu; žádné prázdné marketingové fráze; žádné vymyšlené reference\"",
    "}"
  ].join("\n");
}

async function generateOpenAiDraft({ lead, offer, fallback, pageContext, styleGuide, apiKey, model }) {
  const response = await fetch(OPENAI_URL, {
    ...
    body: JSON.stringify({
      ...
      input: [
        {
          role: "developer",
          content: "Jsi Monika Sikorová z Chaty Dr. Hrstky. Piš přirozeně polsky, lidsky, konkrétně a pravdivě. Neodesíláš e-maily, jen připravuješ návrh ke kontrole."
        },
        {
          role: "user",
          content: buildPrompt({ lead, offer, fallback, pageContext, styleGuide })
        }
      ]
    })
  });
...
async function generateGeminiDraft({ lead, offer, fallback, pageContext, styleGuide, apiKey, model }) {
  const response = await fetch(`${GEMINI_URL}/${encodeURIComponent(model)}:generateContent`, {
    ...
    body: JSON.stringify({
      systemInstruction: {
        parts: [
          {
            text: "Jsi Monika Sikorová z Chaty Dr. Hrstky. Piš přirozeně polsky, lidsky, konkrétně a pravdivě. Neodesíláš e-maily, jen připravuješ návrh ke kontrole."
          }
        ]
      },
      contents: [
        {
          ...
          parts: [
            {
              text: buildPrompt({ lead, offer, fallback, pageContext, styleGuide })
            }
          ]
        }
      ],
...
export async function generateAiDraft(lead, offer, options = {}) {
  ...
  const styleGuide = options.styleGuide || await loadStyleGuide();
  const errors = [];
